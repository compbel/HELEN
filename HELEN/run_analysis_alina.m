...
for cnt = 1:length(countries)
    ...
    seqF = fastaread(['Genomic_data' filesep continent filesep 'sequences_' country '_protein.fasta']);
    ...
    refF = fastaread(['Genomic_data' filesep 'reference_s_aa.fas']);
    ...
    outdir = ['HELEN_data1' filesep 'Preprocessed_input_data'];
    if ~exist(outdir, 'dir')
        mkdir(outdir)
    end
    ...
    load(['HELEN_data1' filesep 'Preprocessed_input_data' filesep 'data_' country '_2022.mat']);
    for t = 1:length(time_points)
        time_test = time_points(t);
        outdir = ['HELEN_data1' filesep 'Epistatic_networks' filesep country filesep char(time_test)];
    ...
    outdirRes = ['HELEN_data1' filesep 'pvalues' filesep country];
    if ~exist(outdirRes, 'dir')
        mkdir(outdirRes)
    end
    parfor t = 1:length(time_points)
        time_test = time_points(t)
        netData = load(['HELEN_data1' filesep 'Epistatic_networks' filesep country filesep char(time_test) filesep 'edges.mat']);
        E = netData.E;
        G = graph(E(:,1),E(:,2));
        if max(max(E)) < nMut
    ...
    outdirRes = ['HELEN_data1' filesep 'Densest_subgraphs'];
    if ~exist(outdirRes, 'dir')
        mkdir(outdirRes)
    end
    t = 0;
    ...
    load(['HELEN_data1' filesep 'Preprocessed_input_data' filesep 'data_' country '_2022.mat']);
    outdirRes = ['HELEN_data1' filesep 'Inferred_variants' filesep country];
    if ~exist(outdirRes, 'dir')
        mkdir(outdirRes)
    end
    ...
    save(['HELEN_data1' filesep 'Inferred_variants' filesep country '_densestFixSizeExt.mat'],'stats');
end