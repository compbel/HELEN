function [E,counts] = constructEpisNetworkExt(M,rho)

[X,Y] = meshgrid(1:size(M,2),1:size(M,2));
pairs = [X(:) Y(:)];
episPairs = false(1,length(pairs));
countPairs = zeros(1,length(pairs));
...
parfor pair = 1:length(pairs)
    ...
    if (prob <= thr) && (O_11 >= 1) && (O_10 >= 1) && (O_01 >= 1)
        episPairs(pair) = 1;    
        countPairs(pair) = O_11;
    end
end

counts = zeros(size(M,2),size(M,2));
for p = 1:length(pairs)
    if episPairs(p) == 1
        counts(pairs(p,1),pairs(p,2)) = countPairs(p);
        counts(pairs(p,2),pairs(p,1)) = countPairs(p);
    end
end
E = pairs(episPairs,:);
