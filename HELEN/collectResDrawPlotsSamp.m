...
for c = 1:length(countriesAll)
    ...
    perc_lgcomp_time1 = zeros(size(voc,1),length(time_points));
    ...
    for t = 1:length(time_points)
        netData = load([outfolder filesep 'Epistatic_networks' filesep countriesAll{c} filesep char(time_points(t)) filesep 'edges.mat']);
        E = netData.E;
        G = graph(E(:,1),E(:,2));
        if max(max(E)) < nMut
        ...
        for s = 1:size(voc,1)
            ...
            [~,idx]=ismember(find(voc(s,:)),nonisol);
            H = subgraph(G1,idx(idx>0));
            perc_lgcomp_time(s,t) = numnodes(H)/sum(voc(s,:));
        end
    end
...
signCorrel = signCorrel(ceil(0.05*length(signCorrel)):(end - ceil(0.05*length(signCorrel))));
...
set(gcf, 'Position',  [100, 500, 2500, 700])
...
%             b(i).CData = voc_colors(v,:);
            ...
            if ptSorted(i) == 0
                ...
                if i < length(b)
                    ...
                    bw = b(i).XEndPoints - b(i-1).XEndPoints;
                end
...
exportgraphics(gcf,['Figures' filesep 'pvalueSummary_D' dataset '.png'],'Resolution',600)
%% 
...
%         ptime = find(p_voc_all(s,:)<=0.05,1,'first');
    ...
    exportgraphics(gcf,['Figures' filesep countriesAll{c} '_pvalues_D' dataset '.png'],'Resolution',600)
end 
%% 
...
for s = 1:(size(voc,1))  
    ...
    set(gcf, 'Position',  [100, 100, 1250, 1100])
    count = 1;
    for c = 1:length(countriesAll) 
        axes(ha(count)); 
        title([voc_names{s} ' ' countriesAllPrint{c}],'fontsize',ft)
...
%         ptime = find(p_voc_all(s,:)<=0.05,1,'first');
    ...
    exportgraphics(gcf,['Figures' filesep voc_names{s} '_pvalues_all_D' dataset '.png'],'Resolution',600)
end
