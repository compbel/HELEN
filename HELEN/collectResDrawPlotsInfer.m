...
load([outfolder  filesep 'Preprocessed_input_data' filesep 'data_all_2022.mat']);
...
fig = figure
tile = tiledlayout(8,2);
axes = [];

for c = 1:length(countriesAll)
    country = countriesAll{c}
    load([outfolder filesep 'Inferred_variants' filesep countriesAll{c} '_densestFixSizeExt.mat']);
    ...
    for t = 1:length(time_points)
        ...
        for v = unique(indMaxF)'
            ind = find((indMaxF'==v)&(stats{t}.vocSupport >= eps));
            if ~isempty(ind)
                fData(t,v) = max(valMaxF(ind));
            end
        end
        for v = unique(indMaxSens)'
            ind = find((indMaxSens'==v)&(stats{t}.vocSupport >= eps));
            if ~isempty(ind)
        ...
        if ~isempty(stats{t}.allF)
            for s = 1:10
                if (max(stats{t}.allF(:,s))>=(0.8-eps1)) && (isinf(predictTime(c,s)))
                ...
                if (max(stats{t}.allF(:,s))>=(0.8-eps1)) && (isinf(predictTime1(c,s)))
                    ...
                    % end
                    tm1perc = find(relPrev_voc_all(s,:)>=0.01,1,'first');
                    if isempty(tm1perc)
                        tm1perc = find(relPrev_voc_all(s,:) == max(relPrev_voc_all(s,:)),1,'first');
                    end
                    predictTime1(c,s) = -days(time_points(t)-time_points(tm1perc));
                end
            end
        end
        ...
        if ~isempty(stats{t}.allF)
            for v = 1:length(stats{t}.vocInfer)
                if valMaxF(v)>=0.8-eps1
                    matchDistAllExtend(c,t) = matchDistAllExtend(c,t) + stats{t}.vocSupport(v)*valMaxF(v);
                else
                    ...
                    if ((sum(ind_future)>=mult*sum(ind_past))&&(sum(ind_future)>=mult))||(sum(ind_detected)>0)
                        matchDistAllExtend(c,t) = matchDistAllExtend(c,t) + stats{t}.vocSupport(v)*max(fscore(ind_future|ind_detected));
    ...
    % figure
    axes(c) = nexttile;
    b = bar(axes(c),time_points_adj,Data_sorted,'BarWidth', 1,'FaceColor','flat','EdgeColor','k');
    for i = 1:length(b)
    ...
    xlim([time_points_adj(1)-days(15) time_points(end)+days(15)])
    grid on
    ylabel(tile,'f-score','fontweight','bold','fontsize',8)
    ylim([0 1])
    a = get(gca,'XTickLabel');  
    set(gca,'XTickLabel',a,'FontWeight','bold','FontSize',8)
    set(gcf, 'Position',  [100, 100, 2000, 1300])
...
hold on
voc_leg = [];
for i = 1:(length(voc_colors))
    voc_leg(i) = plot(NaN,NaN,'Color',voc_colors(i,:),'LineWidth',4);
end
hold off
lg = legend(voc_leg,voc_names);
lg.Layout.Tile = 'South'; % <-- Legend placement with tiled layout
exportgraphics(gcf,['Figures' filesep 'inferred_haplotypes_D' dataset '.png'],'Resolution',600)
...
exportgraphics(gca,['Figures' filesep 'specificity_infer_D' dataset '.png'],'Resolution',600) 
...
%     ylabel('f-score','fontweight','bold','fontsize',14)
    ...
    yl = ylim;
    if v == 1
        text(time_points(end)+days(7),0.99*yl(2),'(a)','FontSize',14,'FontWeight','bold','HorizontalAlignment','right','VerticalAlignment','top');
    end
    yticks([0 0.5 0.8]);
end
linkaxes(axes,'x');
...
xl = xlim;
xlim([xl(1) time_points(end) + days(7)]) 
tile.TileSpacing = 'none';
exportgraphics(fig,['Figures' filesep 'inferFscoreAll_D' dataset '.png'],'Resolution',600)
...
exportgraphics(gcf,['Figures' filesep 'inferSummary_D' dataset '.png'],'Resolution',600)
%% 

vocFound = find(sum(~isinf(predictTime),1)>0)
sum(~isinf(predictTime),1)

sum(~isinf(predictTime),2)
median(sum(~isinf(predictTime),2))

sum(~isinf(predictTime(:,1:5)),2)
median(sum(~isinf(predictTime(:,1:5)),2))
